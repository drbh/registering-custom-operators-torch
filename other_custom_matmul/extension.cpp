...
torch::Tensor matmul(const torch::Tensor &A, const torch::Tensor &B, const bool flag)
{
...
TORCH_LIBRARY(other_custom_ops, m)
{
    m.def("matmul(Tensor A, Tensor B, bool flag) -> Tensor", &matmul);
}

TORCH_LIBRARY_IMPL(other_custom_ops, CPU, m)
{
